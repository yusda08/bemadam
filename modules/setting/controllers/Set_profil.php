...
/**
 * Description of Set_kabupaten
 *
 * @author Asus
 */
class Set_profil extends MY_Controller {

    //put your code here
    var $a;
    private $tabel_profil_dinas = 'profil_dinas';
    ...
    public function index() {
        if ($this->a) {
            ...
            $data = $this->layout_back('setting/set_profil_dinas', $record);
            $data['ribbon'] = ribbon('Setting', 'Setting Profil SKPD');
            $this->backend($data);
        } else {
            redirect('Login');
        }
    ...
    public function user() {
        if ($this->a) {
            ...
            $record['aktifitas'] = $this->Model_Auth->aktifitas(array('kd_user' => $this->a['kd_user']));
            $data = $this->layout_back('setting/set_profil_user', $record);
            $data['ribbon'] = ribbon('Setting', 'Setting Profil SKPD');
            $this->backend($data);
        } else {
            redirect('Login');
    ...
    public function editProfil() {
        ...
        $record['row_user'] = $this->Model_Auth->get_user($kd_user);
        $data = $this->load->view('setting/action/edit_profil', $record);
        return $data;
    }

    function update_profilDinas() {
        $url = $this->input->post('url');
        $id = $this->input->post('id');
        $ket = 'Update Profil Kabupaten';
        ...
        if ($imageName != '') {
            rename('assets/img/_thumb/' . $imageName, 'assets/img/' . $imageName);
            $data['logo'] = $imageName;
            $path = '././assets/img/';
            unlink($path . $img);
        }
        ...
        $query = $this->update('id', $id, $this->tabel_profil_dinas, $data);
        if ($query) {
            $this->aktifitas($ket);
    ...
    function crop_ImgUpload() {
        ...
        rename($imageName, 'assets/img/_thumb/' . $imageName);
        echo '<img src="' . base_url('assets/img/_thumb/') . $imageName . '" class="img-thumbnail" />';
    ...
    function update_profil() {
        $url = $this->input->post('url');
        $kd_user = $this->input->post('kd_user');
        $ket = 'Update Profil dengan KODE USER : ' . $kd_user;
        ...
        $img = $this->input->post('img');
        if ($imageName != '') {
            rename('assets/img/_thumb/' . $imageName, 'assets/img/user/' . $imageName);
            $data['foto'] = $imageName;
